...
public class MyBroadcastReceiver2 extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        Toast.makeText(context, context.getResources().getString(R.string.broadcast_message_2), Toast.LENGTH_SHORT).show();

        Bundle bundle = getResultExtras(true);
        String trail = bundle.getString(BREAD_CRUMB);
        trail = (trail == null ? "Start -> " + TAG : trail + " -> " + TAG);
        bundle.putString(BREAD_CRUMB, trail);
        Log.i(TAG, context.getResources().getString(R.string.broadcast_message_2) + " " + trail);
    }
}
